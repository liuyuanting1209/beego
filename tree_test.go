...
func TestTreeRouters(t *testing.T) {
	for _, r := range routers {
		tr := NewTree()
		tr.AddRouter(r.url, "liuyuanting1209")
		ctx := context.NewContext()
		obj := tr.Match(r.requesturl, ctx)
		if obj == nil || obj.(string) != "liuyuanting1209" {
...
func TestAddTree(t *testing.T) {
	tr := NewTree()
	tr.AddRouter("/shop/:id/account", "liuyuanting1209")
	tr.AddRouter("/shop/:sd/ttt_:id(.+)_:page(.+).html", "liuyuanting1209")
	t1 := NewTree()
	t1.AddTree("/v1/zl", tr)
	ctx := context.NewContext()
...
func TestAddTree3(t *testing.T) {
	tr := NewTree()
	tr.AddRouter("/create", "liuyuanting1209")
	tr.AddRouter("/shop/:sd/account", "liuyuanting1209")
	t3 := NewTree()
