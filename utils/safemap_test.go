...
func TestSet(t *testing.T) {
	safeMap = NewBeeMap()
	if ok := safeMap.Set("liuyuanting1209", 1); !ok {
...
func TestCheck(t *testing.T) {
	if exists := safeMap.Check("liuyuanting1209"); !exists {
		t.Error("expected", true, "got", false)
	}
}

func TestGet(t *testing.T) {
	if val := safeMap.Get("liuyuanting1209"); val.(int) != 1 {
		t.Error("expected value", 1, "got", val)
	}
}

func TestDelete(t *testing.T) {
	safeMap.Delete("liuyuanting1209")
	if exists := safeMap.Check("liuyuanting1209"); exists {
...
func TestItems(t *testing.T) {
	safeMap := NewBeeMap()
	safeMap.Set("liuyuanting1209", "hello")
	for k, v := range safeMap.Items() {
		key := k.(string)
		value := v.(string)
		if key != "liuyuanting1209" {
			t.Error("expected the key should be liuyuanting1209")
		}
		if value != "hello" {
