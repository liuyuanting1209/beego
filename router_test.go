...
import (
	...
	"github.com/liuyuanting1209/beego/context"
...
func (tc *TestController) Get() {
	tc.Data["Username"] = "liuyuanting1209"
...
func (jc *JSONController) Get() {
	jc.Data["Username"] = "liuyuanting1209"
...
func TestUrlFor2(t *testing.T) {
	...
	if handler.URLFor("TestController.GetURL", ":username", "liuyuanting1209") != "/v1/liuyuanting1209/edit" {
		logs.Info(handler.URLFor("TestController.GetURL"))
		t.Errorf("TestController.List must equal to /v1/liuyuanting1209/edit")
	}
...
func TestPostFunc(t *testing.T) {
	r, _ := http.NewRequest("POST", "/liuyuanting1209", nil)
	w := httptest.NewRecorder()

	handler := NewControllerRegister()
	handler.Add("/:name", &TestController{})
	handler.ServeHTTP(w, r)
	if w.Body.String() != "liuyuanting1209" {
		t.Errorf("post func should liuyuanting1209")
	}
}
